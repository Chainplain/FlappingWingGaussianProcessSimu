...
############### OVerall Test Region ############
This_experiemnt_name  = 'GPR'
Desired_Angular_velocity      = '_B_'
...
## optimization func
global State_vec
global gpr_output_vec


mean_X_data9plus = np.load('mean_X_data9plus.npy')
std_X_data9plus = np.load('std_X_data9plus.npy')
...
gpr_output_vec = np.array( [0.0] * 6 )
SO3_Attitude_Controller_u = np.array( [0.0] * 3 )
...
while flapper.step(timestep) != -1:
    ...
    Flapper_OrVec = TheFlapper.getOrientation()
    # print('Flapper_OrVec:', Flapper_OrVec)
    ...
    Flapper_translational_velocity_current =  np.matmul(Flapper_Rotation_current_T,\
         np.matrix([[TheFlapper_velVec[0]],[TheFlapper_velVec[1]],[TheFlapper_velVec[2]]]))
    # Flapper_translational_velocity_current = np.matrix([[TheFlapper_velVec[0]],[TheFlapper_velVec[1]],[TheFlapper_velVec[2]]])
    # print('Flapper_translational_velocity_current:', Flapper_translational_velocity_current)
   ...
   ####-------------Control Tasks-------------------
    ...
    # print('shape:',np.shape(Flapper_Angular_velocity_current))
    #FT observer update
    ...
    p_d_z   =   k_rate_z *  np.sin ( np.pi * t_rate_z * RecordCount * Simulation_Gap )
    v_d_z   =   k_rate_z *  np.pi * t_rate_z * np.cos ( np.pi * t_rate_z * RecordCount * Simulation_Gap )

    d_v_d_z =   - k_rate_z *  np.pi * t_rate_z *  np.pi * t_rate_z * np.sin ( np.pi * t_rate_z * RecordCount * Simulation_Gap )
    ...
    ###sinusoidal   
        ...
        Angle_vel_in_vec = [Angle_vel[0,0], Angle_vel[1,0], Angle_vel[2,0]]
        Trans_vel_in_vec = [Flapper_translational_velocity_current [0,0], 
                            Flapper_translational_velocity_current [1,0], 
                            Flapper_translational_velocity_current [2,0]]
        
        State_vec = Flapper_OrVec + Angle_vel_in_vec + Trans_vel_in_vec
        ...
        u_t_in_body_frame = np.matmul(Flapper_Rotation_current_T, u_t)
        
        
        
        
        gpr_output_vec =  np.array([u_t_in_body_frame[0,0], u_t_in_body_frame[1,0], u_t_in_body_frame[2,0], 
                          SO3_Attitude_Controller. opt_u[0,0],
                          SO3_Attitude_Controller. opt_u[1,0],
                          SO3_Attitude_Controller. opt_u[2,0]])
                          
        # print ('SO3_Attitude_Controller. u[:,0].T:', np.array(SO3_Attitude_Controller. u).flatten())                  
        # SO3_Attitude_Controller_u = SO3_Attitude_Controller_u + 0.01 * np.array(SO3_Attitude_Controller. u).flatten()
        # SO3_Attitude_Controller_u[SO3_Attitude_Controller_u >5] = 5
        # SO3_Attitude_Controller_u[SO3_Attitude_Controller_u<-5] = -5
        # print ('SO3_Attitude_Controller_u:', SO3_Attitude_Controller_u) 
        print ('SO3_Attitude_Controller. u:', SO3_Attitude_Controller. u) 
        print ('SO3_Attitude_Controller. opt_u:', SO3_Attitude_Controller. opt_u) 
        ...
        def control_function(para):
            flap    = para["flap"]  + np.linalg.norm (u_t_in_body_frame[2,0])
            pitch   = para["pitch"] - K_pitch * SO3_Attitude_Controller. u[0,0] 
            roll    =  para["roll"] - K_roll_mix * SO3_Attitude_Controller. u[1,0]
            yaw     = para["yaw"] +  K_yaw * SO3_Attitude_Controller. u[2,0]
            
            X_try   = np.array(State_vec)
            X_input = np.array([flap, pitch, roll, yaw])
        
            X_c = np.concatenate([X_try,X_input],axis=0)
            
            X_c[9:] = (X_c[9:] - mean_X_data9plus) / std_X_data9plus /3
            # print('X_c',X_c)
            
            mu_single, cov_single = gpr.predict(np.array([X_c]))
            # print('mu_single:', mu_single[0,:])
            # print('np.array(gpr_output_vec):', np.array(gpr_output_vec))
            error = mu_single - np.array([gpr_output_vec])
            # print('error:', error)
            # print('np.linalg.norm( error[0,3:6] ) :', np.linalg.norm( error[0,3:6] ) )
            return  - np.linalg.norm( error[0,3:6] ) - 0.2* np.linalg.norm( error[0,0:3] ) \
                    - 0.1 * cov_single [0,0]
        
        torque_mod = 0.1
        search_space = {
                "flap": np.arange(-2, 2, 0.01),
                "pitch": np.arange(-torque_mod, torque_mod, 0.01),
                "roll": np.arange(-torque_mod, torque_mod, 0.01),        
                "yaw": np.arange(-torque_mod, torque_mod, 0.01),
            }
        ...
        opt.search(control_function, n_iter=20, verbosity=[])
        print(opt.best_para)
        
        
        
        # StrokeFreq =  11 / 9.8 * opt.best_para['flap']
        ...
        print('roll_rudder_amplitude:', -K_roll_mix * SO3_Attitude_Controller.u[0,0])
        print('torsion_spring_pitch_offset:', - K_pitch * SO3_Attitude_Controller.u[1,0])
        print('torsion_spring_yaw_offset:',  K_yaw * SO3_Attitude_Controller.u[2,0])
        StrokeFreq =   10 / 9.8* np.linalg.norm (u_t) +  opt.best_para['flap']
        torsion_spring_pitch_offset =  -  K_pitch * SO3_Attitude_Controller.u[1,0] +  opt.best_para['pitch']
        torsion_spring_yaw_offset   =   K_yaw * SO3_Attitude_Controller.u[2,0] + opt.best_para['yaw']
        roll_rudder_amplitude = - K_roll_mix * SO3_Attitude_Controller.u[0,0] +  opt.best_para['roll']
    ...
    # print('Angular_velocity_filter. Get_filtered():',Angular_velocity_filter. Get_filtered())
